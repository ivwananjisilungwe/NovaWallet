...
/**
 * Append-only audit log service for recording all balance changes.
 *
 <p>Writes are asynchronous ({@code @Async}) and non-blocking, using a
 * {@code REQUIRES_NEW} transaction propagation so they never roll back
 * the main business transaction. Supports an {@code @Audited} AOP annotation
 * for automatic method-level auditing.</p>
 */
@Service
